function Env(t, s) {
    return new class {
        ...
        wait(t) {
            ...
            const s = (new Date).getTime(), e = (s - this.startTime) / 1e3, minutes = Math.floor(e / 60), roundedSeconds = Math.round(e % 60);
            const formattedMinutes = String(minutes).padStart(2, '0');
            const formattedSeconds = String(Math.abs(roundedSeconds)).padStart(2, '0');
            // const s = (new Date).getTime(), e = (s - this.startTime) / 1e3;
            this.log("", `🔔${this.name}, 结束! 🕛 耗时：${formattedMinutes}分${formattedSeconds}秒`), this.log(), (this.isSurge() || this.isQuanX() || this.isLoon()) && $done(t)
        }
    }(t, s)
}
